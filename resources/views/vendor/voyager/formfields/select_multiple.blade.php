@if(class_basename($dataTypeContent) == 'Category')
    ...
    @if(isset($options->relationship) && !method_exists( $dataType->model_name, \Illuminate\Support\Str::camel($row->field) ) )
        <p class="label label-warning"><i
                class="voyager-warning"></i> {{ __('voyager::form.field_select_dd_relationship', ['method' => \Illuminate\Support\Str::camel($row->field).'()', 'class' => $dataType->model_name]) }}
        </p>
    @endif
            ...
            @if(isset($options->relationship))
                ...
                @if( method_exists( $dataType->model_name, \Illuminate\Support\Str::camel($row->field) ) )
                    ...
                    @foreach($relationshipOptions as $relationshipOption)
                        ...
                        @if (is_object($relationshipOption))
                            <option value="{{ $relationshipOption->{$options->relationship->key} }}"
                                    @if(in_array($relationshipOption->{$options->relationship->key}, $selected_values)) selected="selected" @endif>{{ $relationshipOption->{$options->relationship->label} }}</option>
                        @else
                            <option value="{{ $relationshipOption[$options->relationship->key] }}"
                                    @if(in_array($relationshipOption[$options->relationship->key], $selected_values)) selected="selected" @endif>{{ $relationshipOption[$options->relationship->label] }}</option>
                        @endif

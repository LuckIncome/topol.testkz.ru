<ul class="{{!isset($isParent) ? 'nav navbar-nav hamb-menu' : 'dropdown-menu'}}">
    @php
    ...
    @foreach ($items as $item)
        ...
        @if(!in_array(class_basename($originalItem) ,['Page','Category']))
            <li class="nav-item {{ $isActive }} {{($originalItem->hasPage || !$originalItem->children->isEmpty()) ? 'dropdown':''}}">
                <a href="{{$originalItem->hasPage ? '#' : url($originalItem->link()) }}" target="{{ $originalItem->target }}" style="{{ $styles }}">

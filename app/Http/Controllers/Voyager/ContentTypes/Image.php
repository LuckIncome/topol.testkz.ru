                ...
                if (isset($this->options->thumbnails)) {
                    foreach ($this->options->thumbnails as $thumbnails) {
                        if (isset($thumbnails->name) && isset($thumbnails->scale)) {
                            ...
                            $imageWeb = InterventionImage::make($file)->resize(
                                $thumb_resize_width,
                                $thumb_resize_height,
                                function (Constraint $constraint) {
                                    $constraint->aspectRatio();
                            ...
                            $image = InterventionImage::make($file)
                                ...
                                ->encode($file->getClientOriginalExtension(), $resize_quality);
                            $imageWeb = InterventionImage::make($file)
                                ->fit($crop_width, $crop_height)
                                ->encode('webp', $resize_quality);
                        }
                        $image->save(storage_path('app/public/') . $path . $filename . '-' . $thumbnails->name . '.' . $file->getClientOriginalExtension());
                        $imageWeb->save(storage_path('app/public/') . $path . $filename . '-' . $thumbnails->name . '.webp');
...
//                        );
//                        Storage::disk(config('voyager.storage.disk'))->put(
//                            $path.$filename.'-'.$thumbnails->name.'.webp',
//                            (string) $imageWeb,
//                            'public'
//                        );
                    }
